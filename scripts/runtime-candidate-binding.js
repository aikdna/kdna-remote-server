...
const NPM_RELEASE_PATH = 'tests/fixtures/runtime-tools/npm-11.17.0.tgz';
const NPM_RELEASE_INTEGRITY =
...
function resolveTrustedNpmInvocation(
  root,
  npmReleasePath = path.join(root, NPM_RELEASE_PATH),
  nodeExecPath = process.execPath,
) {
  assert(path.isAbsolute(nodeExecPath), 'trusted Node executable path must be absolute');
  for (const [file, label] of [[nodeExecPath, 'Node executable']]) {
  ...
  const expectedReleasePath = path.join(rootReal, ...NPM_RELEASE_PATH.split('/'));
  assert(
    typeof npmReleasePath === 'string' && path.isAbsolute(npmReleasePath),
    'trusted npm release path must be absolute',
  );
  const releaseStat = fs.lstatSync(npmReleasePath);
  assert(
    releaseStat.isFile() && !releaseStat.isSymbolicLink() && releaseStat.nlink === 1,
    'trusted npm release must be one regular non-symlink file',
  );
  assert(
    npmReleasePath === expectedReleasePath && fs.realpathSync(npmReleasePath) === expectedReleasePath,
    'trusted npm release must use its exact canonical authority path',
  );
  const releaseBytes = fs.readFileSync(npmReleasePath);
  assert(
    `sha512-${digest(releaseBytes, 'sha512', 'base64')}` === NPM_RELEASE_INTEGRITY,
    'trusted npm release bytes do not match official npm 11.17.0 integrity',
  );
  const entries = readTarFileEntries(releaseBytes);
  const manifests = entries.filter((entry) => entry.path === 'package/package.json');
  assert(manifests.length === 1, 'trusted npm release package manifest is not exact');
  const npmManifest = JSON.parse(manifests[0].bytes.toString('utf8'));
  assert(
    npmManifest.name === 'npm' && npmManifest.version === '11.17.0',
    'trusted npm release identity must be npm 11.17.0',
  );

  const extractionRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'kdna-npm-11.17.0-'));
  try {
    for (const entry of entries) {
      const destination = path.join(extractionRoot, ...entry.path.split('/'));
      assert(isWithin(extractionRoot, destination), 'trusted npm release entry escapes extraction root');
      fs.mkdirSync(path.dirname(destination), { recursive: true, mode: 0o700 });
      fs.writeFileSync(destination, entry.bytes, { flag: 'wx', mode: 0o600 });
    }
    const npmExecPath = path.join(extractionRoot, 'package', 'bin', 'npm-cli.js');
    const npmExecStat = fs.lstatSync(npmExecPath);
    assert(
      npmExecStat.isFile() && !npmExecStat.isSymbolicLink() && npmExecStat.nlink === 1,
      'isolated npm CLI must be one regular non-symlink file',
    );
    let cleaned = false;
...
function readTarFileEntries(artifact) {
  let archive;
  try {
    const input = Buffer.isBuffer(artifact) ? artifact : fs.readFileSync(artifact);
    archive = zlib.gunzipSync(input, { maxOutputLength: 64 * 1024 * 1024 });
  } catch {
...
function assertExactAikdnaLockPackages(packageLock, directNames, boundNames, directVersions) {
  ...
  for (const name of directNames) {
    ...
    assert(
      paths.length === 1,
      `AIKDNA lock package must appear exactly once: ${name} count=${paths.length} paths=[${paths.join(', ')}]`,
    );
    assert(
      paths[0] === `node_modules/${name}`,
      `AIKDNA lock package must be top-level: ${name} path=${paths[0]}`,
    );
  }
...
function verifyCandidateBinding(root) {
  ...
  for (const entry of binding.packages) {
    ...
    assert(
      coreEvidence.evidence_kind === 'candidate_source_pack' &&
    ...
    assert(
      (coreEvidence.pack?.status === 'candidate_source_pack_not_registry_artifact' ||
        ...
        coreEvidence.pack?.filename === 'aikdna-kdna-core-0.21.0.tgz' &&
        ...
        coreEvidence.pack?.sha1 === digest(bytes, 'sha1', 'hex') &&
        coreEvidence.pack?.sha256 === entry.sha256 &&
        coreEvidence.pack?.sha512 === entry.integrity &&
        coreEvidence.pack?.reproducible_runs === 2,
