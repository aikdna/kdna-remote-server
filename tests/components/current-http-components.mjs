...
async function streamCall(handler,body,options={}){
 let local,resultError=null;const pending=new Set();
 const server=http.createServer((req,res)=>{
  ...
  const p=handler(req,res).then(r=>{local=r;}).finally(()=>pending.delete(p));pending.add(p);
 });
 server.listen(0,'127.0.0.1');await new Promise((resolve,reject)=>{server.once('listening',resolve);server.once('error',reject);});
 const address=server.address();assert.equal(address.address,'127.0.0.1');assert.equal(address.family,'IPv4');const url=`http://127.0.0.1:${address.port}${options.url??'/read'}`;
 let response,bytes=Buffer.alloc(0),parsed=null;
 try{
  try{response=await fetch(url,{method:'POST',headers:{'content-type':'application/json',authorization:options.bearer??'SYNTHETIC-COMPONENT-BEARER'},body:JSON.stringify(body)});assert.equal(response.url,url);bytes=Buffer.from(await response.arrayBuffer());try{parsed=JSON.parse(bytes);}catch{}}
  catch(error){resultError=String(error);if(!options.breakSink)throw error;}
  await Promise.all([...pending]);
  return {status:response?.status??0,bytes,body:parsed,local,headers:response?Object.fromEntries(response.headers):{},actualEndpoint:url,fetchError:resultError};
 }finally{server.closeAllConnections();await new Promise(resolve=>server.close(resolve));await Promise.all([...pending]);}
}
...
for(const name of names){
 ...
 for(const item of view.ir.catalog){const req=request(view,item.judgment_id),ar=admitReadRequest(req,control);assert.equal(ar.channel,'admitted_request');const pure=project(ar.admitted_request,admitted.snapshot);assert.equal(pure.status,'projected');const s=await setup(bytes,view);
  try{const result=await streamCall(s.handler,req);assert.equal(result.status,200);compareContent(result.body,pure,view,req.selection);assert.equal(Number(result.body.budget.actual_bytes),result.bytes.length);assert.equal(result.local.envelope.status,'ready');
   ...
   records.push({name,judgment:item.judgment_id,sha256:createHash('sha256').update(bytes).digest('hex'),bytes:result.bytes.length,body:result.body,methodCount:methods.length,policyObservations:s.observations,mode:'actual-public-handler-native-fetch-HTTP',actualEndpoint:result.actualEndpoint});
  }finally{s.dispose();}
 }
}
...
for(const kind of ['default-deny','scope','revoke','replay','wrong-asset','wrong-version','old-tuple','budget','action','sink-failure']){
 ...
 try{
  ...
  records.push({boundary:kind,actualEndpoint:r.actualEndpoint,status:r.status,body:r.body,localChannel:r.local?.channel,retention:s.handler.retentionState()});
 }finally{s.dispose();}
}
const summary={status:'PASS_COMPONENT_HANDLER_HTTP',role,transport:'actual localhost IPv4 HTTP server and native Fetch, no remote acknowledgement or identity proof',records,ready:records.filter(x=>x.body?.status==='ready').length,rejected:3,boundaries:10};
